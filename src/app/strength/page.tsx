              ...
              {program ? (
                <div className="bg-etg-blue/10 border border-etg-blue/20 rounded-xl p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="text-sm font-medium text-etg-blue">{program.block_name} — Week {program.week_number}</div>
                      <div className="text-xs text-white/40 mt-0.5">{program.goal}</div>
                      {program.coach_notes && (
                        <div className="text-xs text-white/30 mt-2 leading-relaxed border-t border-white/8 pt-2">
                          <span className="text-etg-blue/50 font-medium">Dr. Reid: </span>{program.coach_notes}
                        </div>
                      )}
                    </div>
                    <div className="flex flex-col items-end gap-1 flex-shrink-0 ml-4">
                      <div className="text-xs text-white/30">{thisWeekSessions.length}/{programSessions.filter(s => !s.optional).length} mandatory done</div>
                      <div className="text-[10px] text-white/20">Updated {format(new Date(program.updated_at), 'd MMM HH:mm')}</div>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="bg-white/3 border border-white/10 rounded-xl p-4 text-center text-white/30 text-sm">
                  No program loaded. Run the SQL migration and the program will appear here.
                </div>
              )}

              {programSessions.sort((a, b) => a.order - b.order).map((session, i) => {
                const done = thisWeekSessions.find(s => s.day_type === session.name)
                const sets = done ? sessionSets[done.id] || [] : []

                return (
                  <Card key={session.id} className={`border ${done ? 'border-etg-blue/40' : 'border-white/10'}`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${done ? 'bg-etg-blue' : 'bg-white/15'}`} />
                        <div className="text-sm font-medium text-white">{session.name}</div>
                        {session.optional && <span className="text-[10px] text-white/30 border border-white/10 rounded px-1.5 py-0.5">Optional</span>}
                      </div>
                      <div className="flex items-center gap-2">
                        {done && (
                          <div className="flex gap-1.5 items-center">
                            {done.feel && <span className="text-[10px] bg-etg-blue/15 text-etg-blue px-2 py-0.5 rounded-full">{done.feel.split('—')[0]?.trim()}</span>}
                        ...
                        {session.exercises.map((ex, j) => {
                          ...
                          return (
                            <tr key={j} className="border-t border-white/5">
                              <td className="py-1.5 text-white/70 font-medium w-48">
                                {ex.name}
                                {ex.technique_notes && <div className="text-[10px] text-white/25 font-normal">{ex.technique_notes}</div>}
                              </td>
                              <td className="py-1.5 text-white/35">
                                {ex.sets}×{ex.reps} @RPE{ex.rpe}
                                {ex.load_notes && <span className="text-white/20 ml-1">· {ex.load_notes}</span>}
                              </td>
                              {done && (
                                <td className="py-1.5">
                                  {exSets.length > 0 ? (
