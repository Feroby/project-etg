...
const WEEK1_PROGRAM = [
  {
    day: 'Day 1 — Lower (squat focus)',
    exercises: [
      { name: 'Low bar back squat', sets: '4', reps: '5', rpe: '6', notes: '~80-90kg, slow descent' },
      { name: 'Romanian deadlift', sets: '3', reps: '8', rpe: '6', notes: 'Hinge pattern, not heavy' },
      { name: 'Leg press', sets: '3', reps: '10', rpe: '6', notes: 'Full range, controlled' },
      { name: 'Leg curl', sets: '3', reps: '10', rpe: '6', notes: '' },
      { name: 'Dead bug', sets: '3', reps: '10/side', rpe: '—', notes: 'Core stability' },
    ]
  },
  {
    day: 'Day 2 — Upper (bench heavy)',
    exercises: [
      { name: 'Bench press', sets: '5', reps: '3', rpe: '7-8', notes: '~90-92kg, main bench day' },
      { name: 'Incline DB press', sets: '3', reps: '8', rpe: '7', notes: '' },
      { name: 'Barbell row', sets: '4', reps: '6', rpe: '7', notes: 'Horizontal pull balance' },
      { name: 'Face pulls', sets: '3', reps: '15', rpe: '6', notes: 'Shoulder health' },
      { name: 'Tricep pushdown', sets: '3', reps: '12', rpe: '6', notes: '' },
    ]
  },
  {
    day: 'Day 3 — Lower (hinge + squat accessory)',
    exercises: [
      { name: 'Romanian deadlift', sets: '4', reps: '6', rpe: '6-7', notes: 'Slightly heavier than day 1' },
      { name: 'Goblet squat', sets: '3', reps: '8', rpe: '5', notes: 'Light, pause at bottom — mobility' },
      { name: 'Walking lunges', sets: '3', reps: '8/leg', rpe: '6', notes: '' },
      { name: 'Leg curl', sets: '3', reps: '10', rpe: '6', notes: '' },
      { name: 'Ab wheel / plank', sets: '3', reps: '—', rpe: '—', notes: '' },
    ]
  },
  {
    day: 'Day 4 — Optional: Bench volume + accessories',
    exercises: [
      { name: 'Bench press', sets: '4', reps: '5', rpe: '6-7', notes: '~82-85kg, volume not intensity' },
      { name: 'Overhead press', sets: '3', reps: '8', rpe: '7', notes: '' },
      { name: 'Lat pulldown', sets: '3', reps: '10', rpe: '6', notes: '' },
      { name: 'Bicep curl', sets: '3', reps: '12', rpe: '6', notes: '' },
      { name: 'Tricep superset', sets: '3', reps: '12', rpe: '6', notes: '' },
    ]
  },
]
    ...
    <div className="flex h-screen bg-[#0a0a0a] overflow-hidden">
      ...
      <main className="ml-52 flex-1 overflow-y-auto p-6">
        <div className="max-w-5xl mx-auto">
          ...
          <Tabs
            tabs={[{ id: 'log', label: 'Log session' }, { id: 'program', label: 'Week 1 program' }, { id: 'chat', label: 'Chat' }, { id: 'trends', label: 'Trends' }, { id: 'history', label: 'History' }]}
            active={tab}
            onChange={setTab}
          />
          ...
          {tab === 'log' && (
            <Card accent="blue">
              <form onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <Select label="Session day" id="day_type" options={[
                    { value: 'Day 1 — Lower (squat focus)', label: 'Day 1 — Lower (squat focus)' },
                    { value: 'Day 2 — Upper (bench heavy)', label: 'Day 2 — Upper (bench heavy)' },
                    { value: 'Day 3 — Lower (hinge + squat accessory)', label: 'Day 3 — Lower (hinge + squat accessory)' },
                    { value: 'Day 4 — Optional upper + accessories', label: 'Day 4 — Optional upper + accessories' },
                  ]} />
                  <Select label="How did it feel?" id="feel" options={[
                    { value: 'Strong — exceeded expectations', label: 'Strong — exceeded expectations' },
                    { value: 'Solid — hit the targets', label: 'Solid — hit the targets' },
                    { value: 'Average — got through it', label: 'Average — got through it' },
                    { value: 'Weak — struggled today', label: 'Weak — struggled today' },
                    { value: 'Incomplete — cut short', label: 'Incomplete — cut short' },
                  ]} />
                </div>
                <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <Textarea label="Notes — mobility, soreness, anything to flag for programming" id="session_notes" placeholder="e.g. Hips tight on warm up, loosened by set 3. Left knee tracking fine..." />
                <Divider />
                <Button color="blue" disabled={submitting} className="w-full">
                  {submitting ? <span className="flex items-center justify-center gap-2"><Spinner />Analysing...</span> : 'Submit to strength coach'}
                </Button>
              </form>
          ...
          {tab === 'program' && (
            <div className="space-y-4">
              <div className="bg-etg-blue/10 border border-etg-blue/20 rounded-xl p-4 mb-2">
                ...
                <div className="text-xs text-white/50 leading-relaxed">Goal this week is movement quality, not load. RPE 6 means 4 reps left in the tank. If it feels heavier, drop the weight. No conventional deadlift until week 3.</div>
              </div>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-white/30 uppercase tracking-wider">
                        ...
                        <th className="text-left pb-2 font-medium">Sets</th>
                        <th className="text-left pb-2 font-medium">Reps</th>
                        <th className="text-left pb-2 font-medium">RPE</th>
                    ...
                    <tbody>
                      {day.exercises.map((ex, j) => (
                        <tr key={j} className="border-t border-white/5">
                          <td className="py-1.5 text-white/80">{ex.name}</td>
                          <td className="py-1.5 text-white/60">{ex.sets}</td>
                          <td className="py-1.5 text-white/60">{ex.reps}</td>
                          <td className="py-1.5 text-white/60">{ex.rpe}</td>
                          <td className="py-1.5 text-white/40">{ex.notes}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {tab === 'chat' && (
            <Card accent="blue" className="h-[560px] flex flex-col p-0 overflow-hidden">
              <div className="p-3 border-b border-white/8">
                <div className="text-xs text-white/40">Full conversation with Dr. Reid — programming questions, technique, progression, next week planning</div>
              </div>
          ...
          {tab === 'trends' && (
            <div className="space-y-4">
              ...
              <Card>
                <TrendChart data={durData} dataKey="duration" color="#185FA5" label="Session duration (min)" unit="min" />
              </Card>
            </div>
          )}
                ...
                <Card key={s.id}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-2">
                      <Badge color="blue">RPE {s.rpe}/10</Badge>
                      <Badge color="blue">{s.feel?.split('—')[0]?.trim()}</Badge>
                    </div>
                  </div>
                  ...
                  {s.coach_output && (
                    <div className="text-xs text-white/50 leading-relaxed border-t border-white/8 pt-2">{s.coach_output}</div>
                  )}
                </Card>
              ))}
            </div>
