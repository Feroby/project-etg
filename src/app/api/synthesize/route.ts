...
// Safely fetch synthesis history — table may not exist yet if SQL migration hasn't been run
async function fetchSynthesisHistory() {
  try {
    const { data, error } = await supabase
      .from('synthesis_history')
      .select('overall_score, headline, snapshot_weight, snapshot_hrv, snapshot_avg_protein, created_at')
...
// Safely save synthesis history — silently skip if table doesn't exist
async function saveSynthesisHistory(payload: any) {
  try {
    await supabase.from('synthesis_history').insert(payload)
  } catch {
    // Table doesn't exist yet — run synthesis-history-schema.sql in Supabase to enable
  }
}
...
export async function POST(req: NextRequest) {
  try {
    const [
      ...
      fetchSynthesisHistory(),   // safe — won't crash if table missing
    ...
    const program = (programResult as any)?.data || null
      ...
      benchProgression: (() => {
        ...
        sessions.forEach(s => {
          const sets = (sessionSets[s.id] || []).filter((st: any) => {
            ...
            return name.includes('bench') && !name.includes('incline') && !name.includes('decline') && !name.includes('db') && !name.includes('dumbbell') && st.weight
          })
      ...
      squatProgression: (() => {
        ...
        sessions.forEach(s => {
          const sets = (sessionSets[s.id] || []).filter((st: any) => {
            const name = (st.exercise_name || '').toLowerCase()
            const isBackSquat = (
              name.includes('back squat') || name.includes('low bar') || name.includes('high bar') ||
              (name.includes('barbell') && name.includes('squat'))
            ) && !name.includes('goblet') && !name.includes('split') && !name.includes('bulgarian') && !name.includes('front')
...
Return ONLY valid JSON, no markdown fences:
{
  ...
  "next_checkpoint": "<what to watch in next 7 days>"
    ...
    return NextResponse.json(
      { report, generatedAt: new Date().toISOString(), directives: newDirectives || [] },
